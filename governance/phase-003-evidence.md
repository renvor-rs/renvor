...
| 4 | Compatibility rows exercised | **Linux, macOS, and Windows × pinned MSRV (1.94.0) and current stable — six platform/toolchain contexts, all six passing for the pull-request candidate integration tree.** That tree is GitHub's synthetic merge commit for PR #28, built from base `4d651807afb72e5f8904c896e5c9297b6875bb9e` and reviewed head `68295ef6f71bbe6b87d6b9989f1f1fd948f64d29` — `pull_request` workflows check out `refs/pull/<n>/merge`, so that candidate is what the contexts tested, even though GitHub records the check runs against the head SHA. **The synthetic merge commit's own SHA is not recorded here**: GitHub no longer serves `refs/pull/28/merge` for a closed pull request, and a value that cannot be read is not invented. The base is an ancestor of that head, so the candidate tree is the head tree `40fa31ca33f7f0c2f8499ee7773c891868f000fe`; the squash commit `01327b1` carries that identical tree, and **`main`'s own push run at `01327b1` — not an assumption carried over from the pull request — is what confirms the claim on `main`**, with all six contexts green there. **Of those six, only the two Linux contexts — `verify (1.94.0)` and `verify (stable)` — are in `main`'s required-status-check list** (which also contains the non-platform `security` and `docs` checks); the four macOS and Windows `platform (…)` contexts run on every pull request and count as evidence, but are **not** branch-protection-required checks. Normative policy: [`contracts/support-policy.md`](../contracts/support-policy.md) — the sole current authority, carrying the three-platform claim under [ADR-0011](../decisions/0011-support-linux-macos-and-windows.md). Human-facing summary: [`SUPPORT.md`](../SUPPORT.md). Implementation: [`ci.yml`](../.github/workflows/ci.yml). |
