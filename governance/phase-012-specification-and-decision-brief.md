...
| `rustup` off `PATH` **and not locatable**; the isolated identification probe answers in rustup's words | — | **refused before any proxy runs in the pinned directory** (FR-012-7a step 4) | 5 `tool_missing`, `details.tool = "rustup >= 1.28.1"`, `details.reason = proxy_unidentified` | none | the refusal and the remedy (put `rustup` on `PATH`); **no download**, no proxy ran with an absent toolchain |
