...
**Revised again**: 2026-09-07 (decision round 2), by new commits on the same branch after `origin/main` (`7281e4f`) was merged into it: the maintainer's approvals of **U-1, U-7, U-9, U-10** and the five plan corrections (B1 sequencing; the evidence mechanism FR-012-7d and U-2; evidence freshness FR-012-5d; hidden-proxy safety FR-012-7a/7c; local capacity) are recorded in **§4.3** and applied in place. The 2026-09-07 first-round text is `ac535ee`.
**Status**: **SPECIFICATION — planning approval requested.** The maintainer's dispositions of 2026-09-07 on **D-L1-1 … D-L1-7** and **D-L2-2 … D-L2-10** are recorded in §4 and applied throughout; the second round of 2026-09-07 (U-1, U-7, U-9, U-10 approved; U-2 corrected and still open; U-3 … U-6 and U-8 still pending; the five corrections) is §4.3. **A-0 (approval of this specification and its plan) remains pending** until this corrected revision is validated and the maintainer approves it explicitly. This file, its companion task plan, and the carry-over plan implement nothing, change no contract, template, test, or workflow, accept no decision record, close no limitation, and grant no waiver. **L-1 and L-2 stay open** until implementation and validation prove the exact closure conditions in §9.8 and §5.9. Nothing is tagged, released, published, or deployed. The cheat sheet stays local and untracked.
...
| **AC-012-6** | unit control: pin `X` + `RUSTUP_TOOLCHAIN=Y` → `Y`, `environment`; unset → `X`, `toolchain_file` | **not met** | needs the resolution probe (FR-012-7) and the selection controls. → T-012-10 (C-sel-4) |
...
- **Observed** (job logs read through the GitHub API on 2026-09-06): three stable jobs — `verify (stable)` on `3742ec1` (run `34025561472`, job `101465783525`) and on `5acb492` (run `34018208859`, job `101445807521`), `platform (macos-latest, stable)` on `3742ec1` (job `101465783728`) — and one control, `docs` on `3742ec1` (job `101465783662`, given `1.94.0` by input on purpose). Then the five heads of #64 (§0.2) and the merge on `main`, `7281e4f`.
- **Inferred**: the exposure window **2026-08-11 (`98a4e2c`, the day the pin landed; `ci.yml` followed in `e41ec4f` at 2026-08-11 23:39 UTC) → 2026-09-06 (`7281e4f`)**. The inference rests on configuration history, not on reading every run: `rust-toolchain.toml` changed in no commit after `98a4e2c`; `ci.yml` changed eleven times before #64 and introduced `RUSTUP_TOOLCHAIN` only in #64 (`git log -S RUSTUP_TOOLCHAIN -- .github/workflows/ci.yml` names `3107e38` and `72050b0` alone on the pull request's branch `ci/stable-legs-run-stable`, and the squash `7281e4f` alone on `main`); sixty-four first-parent commits (fifty-five pull-request merges) lie in the window. Any "verified on stable" sentence sourced from CI in that window describes a second MSRV run; the local `cargo +stable xtask verify` legs recorded in the evidence files were genuine (`+stable` sets `RUSTUP_TOOLCHAIN`).
...
### 4.3 Implementation choices decided on 2026-09-07 (round 2), and the five corrections

The maintainer decided four of the task plan's unresolved choices and required five corrections. Each row names where the decision or correction is applied; **U-2 is corrected, not approved**, and every other `U-n` stays pending in the task plan §4 — nothing is approved by implication.

| # | Decision (the maintainer's terms) | Applied in |
|---|---|---|
| **U-1** | **APPROVED — `record_unsupported` exits 3.** An unsupported *input* record is a validation failure (C-1's row 3), not a missing environment tool (row 5). The named reason and the version details (`details.record_version`, `details.supported`) are one string across the contracts, the JSON schema, the `tests/json` fixture, and the help text. | FR-012-5b, §5.8, §6.3, §6.4; task plan T-012-05 |
| **U-7** | **APPROVED for prerelease documentation.** A framework artifact addressed by **full commit SHA and content digest**, with **verified provenance**, satisfies the prerelease immutable-input requirement. The importer verifies the digest, the source repository, the source commit, and the approved producing workflow identity; a filename, a short SHA, or a successful `--repo` verification alone is insufficient. The archive, manifest, checksums, and verification material are preserved for the documented retention period; a rebuilt artifact is never substituted silently after expiry or deletion; every page states that it is a prerelease snapshot, not a signed release; the policy amendment this needs is recorded explicitly, and the decision is no longer labelled an assumption. | FR-012-D14, FR-012-D15, §10.8, §6.8; task plan T-012-33, T-012-36, A-2 |
| **U-9** | **APPROVED — measurement first; no `ConnectionString` redesign in the TLS proof.** T-012-26 measures whether the existing URL parameters carry certificate **and** hostname verification to both database engines through both adapters. If they do, the bounded TLS proof uses them and 010/L-15 is retained. A credential-bearing URL is never printed. If a required setting cannot reach a driver, the exact gap is reported and the smallest API change is proposed separately; verification is never downgraded silently, and a `ConnectionString` redesign is not folded into B3. | WI-012-5 (§11), §12 (010/L-15); task plan T-012-26, B3 |
| **U-10** | **APPROVED — the second toolchain is provisioned explicitly on both CI `verify` legs.** The required components are installed deliberately, `RUSTUP_AUTO_INSTALL=0` stays in force during verification, and a step proves that the controls use two distinct compiler identities. **This does not authorise local toolchain installation.** | §5.4 (Controls), FR-012-7d controls; task plan T-012-10, B1, §3 |

| Correction | What was wrong on `ac535ee` | Applied in |
|---|---|---|
| **1 — B1 sequencing** | B1a shipped the rendered pin, the version-2 record with `[verified_with]`, and the contract revisions **before** B1b's seal, resolution preflight, no-provisioning protection, and legacy handling existed. | Task plan §2: B1a and B1b are one mergeable unit **B1**; B1c becomes the follow-up **B1f**; the only work that may precede B1 is strictly inactive (ADR-0038 proposed; the T-012-08m measurement) |
| **2 — U-2 and FR-012-7d** | The probe crate read the build-script `RUSTC` directly, which does not see compiler wrappers, and a separate workspace can differ from the checks in Cargo configuration, target, runner, and workspace-wrapper behaviour; U-2 recommended it as if it were equivalent to the real invocations. | FR-012-7d is now an evidence requirement tied to the real verification invocations, with the configured/observed distinction, the wrapper trust limitation, per-run freshness with a stale-output control, exclusively created storage, and controls (i)–(ix); the probe crate is withdrawn as the mechanism; **U-2 stays open**, decided on T-012-08m's measurement |
| **3 — evidence freshness** | `manifest_digest` compared the record's own `[[file]]` entries, not file contents; "N generate operations since" had no counter behind it; FR-012-10a said every `generate` verifies, contradicting FR-012-5a/10c. | FR-012-5d (`tree_digest`, scope, additions and deletions, freshness distinct from ownership, the manual-edit control); FR-012-10a/10c reconciled; §5.8; task plan T-012-05 |
| **4 — hidden-proxy safety** | With `rustup` off `PATH`, FR-012-7b ran the resolved `rustc` in the pinned directory and FR-012-7c ran it with an absent toolchain **before** any rustup version was known. | FR-012-7a (identify before invoking; step (2) — the floor check itself runs under the isolation, because `rustup --version` resolves the active toolchain; the isolated identification probe; explicit refusal `proxy_unidentified`), FR-012-7b, FR-012-7c, §5.7, §5.8; task plan T-012-07 with stub-only tests |
| **5 — capacity** | The plan sized a local full gate from two 1.2 GB partial-run directories and called 19 GiB sufficient, while the last full local gate measured about 56 GB. | Task plan §3, with the measurements of 2026-09-06 23:42 UTC and no cache deletion; A-7 |
...
identity_source = "observed"     # observed: the compiler Cargo launched during the checks (FR-012-7d); never "configured"
...
| # | Requirement |
|---|---|
| **FR-012-4** | `[verified_with]` is **measured, never derived**: its values are the evidence of FR-012-7d, taken from the five checks themselves as they ran in the staged (or scratch) tree under the sealed environment; nothing in the table is inferred from `PATH`, from the generator's own process environment, or from a configuration value the checks did not act on. A record written before verification, from the generator's own process environment, or from any cache (`.rustc_info.json`, a previous record) is a test failure. |
...
| **FR-012-5b** | **Reader dispatch (D-L2-9).** A reader reads `record_version` first: **absent** → the record is a legacy (version 1) record: accepted; `[toolchain]` and `[verified_with]` are reported as *unknown*, never filled in; **2** → the whole document is validated strictly (`deny_unknown_fields` within the version); **greater than the reader knows** → refused by name — `record_unsupported`, `details.record_version`, `details.supported = 2` — **before any file is planned or modified** — **exit 3** (C-1's row 3, validation failure: an unsupported *input* record, not a missing environment tool; **U-1, approved 2026-09-07**). The reason string `record_unsupported` and the two `details` keys are the same text in C-1's table, C-2's error-code registry, the `tests/json/record_unsupported.json` fixture, and the help and README sentences that name them; T-012-05 asserts the four agree. |
...
| **FR-012-5d** | **Evidence freshness (D-L2-10): the verified tree, compared by contents.** At the verification — after the five checks pass, before the manifest is written — the generator computes `tree_digest` = SHA-256 over the sorted lines `<relative path>\0<sha256 of the file's bytes>\n` for **every regular file in scope found by walking the staged (or scratch) tree**, and records it with `tree_scope = 1` and `verified_at`. **Scope (`tree_scope = 1`)** — what the five checks compile or read: `Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`, `build.rs`, `renvor.toml`, `.cargo/config.toml`, and every file under `src/`, `tests/`, `benches/`, `examples/`, and `migrations/`; **excluded**: `README.md`, `Dockerfile`, `.dockerignore`, `.gitignore`, `.env*`, the `.renvor/` directory (the record and the manifest themselves), `target/`, and every other path. A symlink in scope is recorded as `<path>\0symlink:<link text>\n`, never followed. **Additions and deletions count**: the scope is a pattern over the tree, not the `[[file]]` list, so a file added under `src/` or deleted from `tests/` changes the digest. **Freshness is not ownership**: C-4's managed blocks decide what the generator owns and may rewrite; for freshness the whole file's bytes count — an edit inside a managed block and an edit outside one both change compiled source, and both make the evidence historical; ownership never hides a change. `renvor check` recomputes the digest over the current working tree under the recorded `tree_scope` (a scope number it does not know is `record_unsupported`, exit 3) and compares: equal → `verified_with: current`; different → `verified_with: historical — the tree verified at <verified_at> (<operation>) is not the current tree; not proof of the current tree`, and `--output json` carries `data.verified_with.historical = true`. **No count of operations is printed**: nothing in the tree records how many operations ran since the verification, and a number nothing supports is not printed. Earlier evidence is **retained**, never deleted and never re-dated. **Controls** (T-012-05): an untouched tree → current; one byte changed in `src/main.rs` **inside** a managed block with the record untouched → historical; one byte changed **outside** a managed block with the record untouched → historical; a file added under `src/` → historical; a file deleted from `tests/` → historical; `README.md` edited → still current; `generate resource` → historical (the files it adds are in scope); `generate auth` → current again (it verifies and writes a new `tree_digest`). |
...
### 5.3 The sealed environment, the identification and resolution probes, and the evidence (FR-012-6 … FR-012-8, SR-012-1 … SR-012-5)
...
| # | Requirement |
|---|---|
| **FR-012-6** | The sealed environment sets `RUSTUP_AUTO_INSTALL=0` **unconditionally** and omits `RUSTUP_DIST_SERVER` and `RUSTUP_UPDATE_ROOT` for every child it spawns — the five checks, the floor check and the identification probe of FR-012-7a, the resolution probe of FR-012-7b, whatever mechanism FR-012-7d's evidence uses, `rustfmt`, and `doctor`'s probes. The three existing seal tests (`verification_runs_in_a_sealed_environment`, `a_proxy_credential_never_reaches_the_sealed_environment`, `a_childs_output_is_reported_without_url_credentials_or_control_characters`) are unchanged; a fourth asserts the forced value and the two absences. |
| **FR-012-7a** | **Identify before invoking.** Nothing runs a proxy in a pinned directory, or with an absent toolchain, until that proxy's rustup version is known to honour the no-install guarantee. The order, before anything is staged: **(1) locate rustup without executing a proxy** — `rustup` on `PATH`; else a `rustup` beside the `rustc` that `PATH` resolves; else `$CARGO_HOME/bin/rustup`; else `~/.cargo/bin/rustup`. **(2) the floor** — for a located `rustup`, `rustup --version`, run **under the isolation of step (4)** (an exclusively created empty working directory with no toolchain file above it, `RUSTUP_HOME` and `CARGO_HOME` pointed at exclusively created empty directories, `RUSTUP_TOOLCHAIN` unset, `RUSTUP_AUTO_INSTALL=0`, `RUSTUP_DIST_SERVER` and `RUSTUP_UPDATE_ROOT` set to an unroutable loopback address in this child only, a bounded timeout) — because `rustup --version` also resolves the active toolchain to report its `rustc` (rustup 1.29.0 prints `the currently active rustc version is …` on stderr — measured on 2026-09-07 — and before 1.28.0 that resolution installed an absent file- or environment-named toolchain: `find_or_install_override_toolchain_or_default` in `rustup_mode.rs`'s `--version` branch), so it is never run in a pinned directory or with a toolchain named before its version is known. Only the first stdout line is parsed: unparseable, or below **1.28.1**, is `tool_missing`, exit 5, `details.tool = "rustup >= 1.28.1"`, and no proxy is invoked (asserted by a test whose `PATH` has no `cargo`). **(3) classify `rustc` and `cargo` without running them in the pinned directory** — each is a proxy of the located rustup when it is the same file after symlinks are followed (same device and inode on Unix; identical bytes on Windows, where the proxies are copies); a proxy of a rustup at or above the floor is *identified*, and FR-012-7b may run. **(4) otherwise, the isolated identification probe** — the binary is run once with `-vV` in an exclusively created empty directory with `RUSTUP_HOME` and `CARGO_HOME` pointed at exclusively created empty directories, `RUSTUP_TOOLCHAIN` unset, `RUSTUP_AUTO_INSTALL=0`, `RUSTUP_DIST_SERVER` and `RUSTUP_UPDATE_ROOT` set to an unroutable loopback address **in this child only** (a belt-and-braces control, not a seal variable), and a bounded timeout. Nothing is pinned or selected in that directory, so the probe requests no toolchain and no rustup release has anything to install there. A bare compiler ignores every `RUSTUP_*` variable and prints its identity → *bare*; FR-012-7b may run with `proxy = false`. An answer in rustup's own words (no default toolchain, nothing installed, `rustup` named) → *a proxy whose rustup could not be located*: **refused explicitly**, `tool_missing`, exit 5, `details.tool = "rustup >= 1.28.1"`, `details.reason = proxy_unidentified`, `details.remedy` = put the `rustup` that owns these proxies first on `PATH`; **FR-012-7b does not run**. Anything else, or the timeout → `project_verification_failed`, `details.reason = compiler_identity_unreadable`. The no-install claim of step (4) is stated and tested with stubs, not measured against old rustup releases, which are not installed for this purpose (U-10). **Tests never allow a real download**: a stub `rustc` that behaves as an old proxy — it "installs" (writes a marker file and connects to the dist address) whenever it runs where a toolchain file names an absent channel — proves the generator never runs it there: the marker is absent, the stub's recorded invocations are only the isolated one, the connection never happened, and the exit is 5 with `proxy_unidentified`. |
| **FR-012-7b** | **The resolution probe runs only after FR-012-7a has identified the tools** — a proxy of a rustup at or above the floor, or a bare compiler; an unidentified binary never reaches it. It is `rustc -vV` and `cargo -vV`, run inside the directory whose selection is being measured (§5.4), under the seal, followed by `rustup show active-toolchain` when rustup is located. rustup's "is not installed" text is the refusal **`tool_missing`, exit 5**, `details.tool = "rustup toolchain <channel>"`, `details.remedy = "rustup toolchain install <channel> --component rustfmt --component clippy --profile minimal"`, before any check runs and before anything is placed. A resolved compiler older than `rust_version` is the same refusal with `details.tool = "rustc >= <msrv>"` (cargo would refuse later; the generator refuses first, by name). A toolchain that resolves but lacks `rustfmt` or `clippy` (`rustfmt --version`, `cargo clippy --version` under the same seal) is `tool_missing` naming the component and the `rustup component add` remedy. `selected_by` is taken from `rustup show active-toolchain`'s attribution text (the strings for the rustup versions named in `support-policy.md` are pinned by a test; unknown text records `unknown`, never a guess); the path rustup prints is not recorded. |
| **FR-012-7c** | **Proxy detection (D-L2-6).** Hiding `rustup` from `PATH` proves nothing about `rustc` and `cargo`, which may still be rustup's proxies (hard links or copies of the rustup binary that read `RUSTUP_HOME` themselves). The classification is FR-012-7a's steps (3) and (4): `proxy = true` when the resolved `rustc` is the same file as a located `rustup`; `proxy = false` when the isolated identification probe printed a compiler identity. The uninstallable-name confirmation — the resolved `rustc` run once with `RUSTUP_TOOLCHAIN=<a name that cannot be installed>` (the identity script's pattern, `RUSTUP_AUTO_INSTALL=0`, the dist variables absent) — runs **only on an identified proxy at or above the floor**, as the in-run witness that this rustup answers "is not installed" and downloads nothing; it is **never** run on an unidentified binary. `selected_by = "no_rustup"` is recorded **only** when `proxy = false`; a proxy with `rustup` off `PATH` is attributed by the file/override/default it actually used, and the notice says so. |
| **FR-012-7d** | **The evidence (D-L2-3): the compiler observed executing the verification — not one configured, and not one inferred from a separate build.** The record's `rustc_*` fields, `identity_source`, and the `rustc_override`/`wrapper` booleans must be evidence **tied to the real verification invocations** — the five checks `in_staging_with` runs on the staged (or scratch) tree — and the mechanism that produces it is **U-2, which stays open**: nothing below approves a mechanism, and none is described as equivalent to another. **What any mechanism must satisfy** — **(a) observed, not configured.** A *configured* compiler is what Cargo would resolve from `RUSTC`, `build.rustc` in its configuration hierarchy, and the wrapper settings; an *observed* compiler is the binary Cargo actually launched during the checks. The record carries the observed identity with `identity_source = "observed"`; a run that produced no observation (every unit `Fresh`, no compiler launched) is `project_verification_failed`, `details.reason = compiler_identity_unobserved` — never a fallback to the configured value or to `PATH`. **(b) wrappers are seen, not bypassed.** The evidence reflects the launch chain Cargo used — `RUSTC_WRAPPER`, `RUSTC_WORKSPACE_WRAPPER`, `build.rustc-wrapper`, `build.rustc-workspace-wrapper` — recording their presence as booleans and taking the identity from the compiler binary in that chain. **Trust limitation, retained explicitly (SR-012-3)**: an operator-controlled wrapper runs with the operator's rights and may substitute anything; the record states what Cargo launched, not what the wrapper did. **(c) the checks' own Cargo configuration.** The evidence comes from the same working directory, `CARGO_HOME`, target directory, `--target`, runner, and configuration-file hierarchy as the checks. A separate workspace can differ in every one of those and in workspace-wrapper behaviour, and a build-script `RUSTC` read does not see wrappers: the 2026-09-07 first-round probe crate under `.renvor/probe/` is therefore **withdrawn as the mechanism** and may at most corroborate. **(d) fresh per run, despite shared caches.** `RENVOR_TEST_TARGET_DIR` and any shared `CARGO_TARGET_DIR` reuse artifacts across runs; the evidence carries this run's staging nonce and is produced during this run. **(e) exclusively created storage.** Any temporary file, directory, or probe workspace a mechanism needs is created with create-new semantics under this run's staging directory (`<staging>/.renvor-evidence-<nonce>/`), removed only if this run created it, and never placed; **a user's `.renvor/probe`, or any other pre-existing path, is never read, overwritten, or removed**. **(f) sanitized** per FR-012-7e; `.rustc_info.json` is never read (a cache). **Controls** (T-012-08; the differing identity is U-10's second toolchain in CI, `SKIPPED:` locally without one): **(i)** `RUSTC` in the sealed environment pointing at another installed toolchain's `rustc` while `PATH` resolves the pin — the record names the `RUSTC` one, `rustc_override = true`; **(ii)** `[build] rustc` in the sealed `CARGO_HOME`'s `config.toml` — the same; **(iii)** `[build] rustc` in a `.cargo/config.toml` of an **ancestor** of the staging directory, the hierarchy the checks actually read — the same; **(iv)** a pass-through `RUSTC_WRAPPER` — identity unchanged, `wrapper = true`, the wrapper's path in no stream; **(v)** a pass-through `RUSTC_WORKSPACE_WRAPPER` — the same; **(vi)** a substituting wrapper that executes the other toolchain's `rustc` whatever it is handed — the record names what Cargo launched with `wrapper = true`, so the sentence in (b) is asserted true rather than assumed; **(vii)** the stale-output control of (d): a previous run's evidence naming another compiler, planted in the shared target directory, and the record names the current run's compiler and nonce; **(viii)** the trivial case — `PATH`'s `rustc -vV` and the observed identity agree, `identity_source = "observed"`; **(ix)** a pre-existing `.renvor/probe/keep` with a sentinel survives byte-identical, and a pre-existing `.renvor-evidence-*` directory that this run did not create is neither reused nor removed. **Candidate mechanisms for U-2** (measured by **T-012-08m** against (a)–(f) and (i)–(ix) before any is chosen; the measurement goes to the maintainer under A-6's reporting pattern, and T-012-08 implements only the decided one): **(1)** the `Running \`… rustc …\`` lines the checks print under `-vv`, giving the launched chain, plus one `-vV` of the launched compiler binary under the same seal — an observed launch, with the identity of that binary; freshness needs at least one non-`Fresh` unit per run; **(2)** `cargo rustc -p <crate> -- -vV` inside the staged tree under the seal — Cargo's configured resolution, wrappers included, but **not** an observation of the checks; **(3)** an observing `RUSTC_WORKSPACE_WRAPPER` supplied by the generator that records what it is asked to run and then runs it unchanged — an observation, but its composition with an operator's own wrapper settings is unproven and it must never displace them; **(4)** the fingerprint and dep-info files the checks write under the target directory — hashes, not identities, so at most a freshness witness. |
...
| **SR-012-4** | The generator never runs a rustup command that lists, installs, updates, or sets a default; it runs proxies (`rustc`, `cargo`, `rustfmt`, `cargo clippy`) only after FR-012-7a has identified them, and `rustup --version` (of a located `rustup`, on `PATH` or beside the proxies, under FR-012-7a step (2)'s isolation) and `rustup show active-toolchain` only, all under FR-012-6. |
...
| Site | Today | Required |
|---|---|---|
| `generate/verify.rs` `in_staging_with` — the five checks for `renvor new` and for `generate auth`'s scratch copy | sealed (allow-list) | the seal of FR-012-6 plus FR-012-7a/b/c before the checks, and FR-012-7d's evidence taken from the checks themselves |
...
| the identification and resolution probes (new) | — | FR-012-7a/b/c — the isolated identification probe runs in an exclusively created empty directory; the resolution probe runs only on identified tools |
...
| **Controls** | **C-sel-1** — pin `X` in the project, `rustup override set Y` on the **parent**: the project's file is closer, so `X`, `toolchain_file`, no notice. **C-sel-2** — the override set on the **project directory itself** with pin `X`: rustup's order puts the override first, so `Y`, `directory_override`, the FR-012-8 notice. **C-sel-3** — a legacy pin-less project under an ancestor with `rust-toolchain.toml` `Z`: `generate auth` resolves `Z` in the project directory and in the sibling scratch copy alike (positive); the pre-change behaviour — a copy under the system temporary directory — resolves the default instead, and the divergence check fails it (negative control, kept as a test of the check itself). **C-sel-4** — `RUSTUP_TOOLCHAIN=Y` with pin `X`: `Y`, `environment` (this is AC-012-6). Each control needs two installed toolchains and is skipped with a `SKIPPED:` line locally and required under `RENVOR_TEST_REQUIRE_TOOLCHAINS=1` in CI (the census pattern). **In CI the second toolchain is provisioned explicitly (U-10, approved 2026-09-07)**: on both `verify` legs a second `dtolnay/rust-toolchain` step installs the *other* leg's toolchain with `components: rustfmt, clippy` — a deliberate installing step (the action's `rustup default` then names the control as the runner default — irrelevant, because the job-level `RUSTUP_TOOLCHAIN` selects and the identity step re-asserts it), while the job-level `RUSTUP_TOOLCHAIN` still selects the leg's own and `RUSTUP_AUTO_INSTALL=0` stays in force for everything the suite runs; `RENVOR_TEST_CONTROL_TOOLCHAIN` names the control; and a step before the suite proves the two identities are distinct (`rustc +<leg> -vV` and `rustc +<control> -vV` differ in release **and** commit) — a control run against an identical identity proves nothing and fails the leg. **No local toolchain installation is authorised**: locally a control whose second toolchain is absent is `SKIPPED:`, never installed. |

### 5.5 Legacy trees and `renvor generate` (FR-012-10, D-L2-10)

| # | Requirement |
|---|---|
| **FR-012-10a** | A project generated at template version 7 or earlier has no pin and a record without `record_version`. `renvor generate` into such a tree **does not refuse** on that account. **Only the operations that verify write evidence** (reconciled with FR-012-5a and FR-012-10c): `generate auth` verifies its scratch copy with whatever resolves, records `[verified_with]` with `operation = "auth"` and `[toolchain]` `pinned = "none"`, `rust_version = "none"`, and prints the FR-012-8 line with `the project pins nothing`; `generate resource` and `generate migration` run `rustfmt` or nothing, verify nothing, print no FR-012-8 line, and leave `[verified_with]` byte-identical. Every `generate` into a legacy tree states once that no `renvor generate toolchain` action exists and that the README of a new project shows the two files to add by hand. No upgrade command is added this phase; L-11 ("nothing upgrades a project with it") stays the row that names the gap. |
...
| **FR-012-10c** | Verification scope is unchanged: `generate resource` and `generate migration` run `rustfmt` or nothing; they never build, and FR-012-5a forbids them to write `verified_with`. Because the files they add are inside FR-012-5d's scope, the earlier evidence becomes historical the moment they run — by the tree digest, not by a counter — and stays historical until an operation that verifies (`generate auth`, or a fresh `renvor new`) writes new evidence. |
...
| rustup version, and whether it is at or above the 1.28.1 floor | `rustup` located as FR-012-7a locates it (on `PATH`, beside the proxies, `$CARGO_HOME/bin`, `~/.cargo/bin`) and `rustup --version` under FR-012-7a step (2)'s isolation, parsed; the proxies classified by FR-012-7a's steps (3)–(4). Below the floor, or a proxy whose rustup cannot be located, the pin probes below are **not run** and the row says `not probed: rustup <v> is below the supported floor` or `not probed: the rustc on PATH is a rustup proxy whose rustup could not be located` |
| what resolves **here** and why | `rustc -vV`, `cargo -vV`, `rustup show active-toolchain`, under the seal of FR-012-6 |
| whether the pin is installed with both components | **only on an identified proxy at or above the floor** (FR-012-7a): `rustc +<pin> -vV`, `rustfmt +<pin> --version`, `cargo +<pin> clippy --version` under the seal — rustup ≥ 1.28.1 with `RUSTUP_AUTO_INSTALL=0` answers "is not installed" by name and downloads nothing; the three lines report present/absent. On a bare toolchain the row says `not applicable: no rustup` |
...
| rustup below 1.28.1, or unparseable | — | **refused before the probe** | 5 `tool_missing`, `details.tool = "rustup >= 1.28.1"` | none | the rustup version and the floor; no proxy ran, and `rustup --version` ran only in the isolated directory |
...
| `rustup` off `PATH`, but `rustc`/`cargo` are proxies of a `rustup` FR-012-7a can locate (beside them, `$CARGO_HOME/bin`, `~/.cargo/bin`) at or above the floor | whatever the proxies resolve (file, override, default) | verifies; attributed correctly | 0 | the real attribution, `proxy = true` | the FR-012-8 line if the result is not the pin |
| `rustup` off `PATH` and not locatable, **or** the located `rustup` is not the proxies' file; the isolated identification probe answers in rustup's words | — | **refused before any proxy runs in the pinned directory** (FR-012-7a step 4) | 5 `tool_missing`, `details.tool = "rustup >= 1.28.1"`, `details.reason = proxy_unidentified` | none | the refusal and the remedy (put the `rustup` that owns these proxies first on `PATH`); **no download**, no proxy ran with an absent toolchain |
...
| `renvor generate` into a legacy tree (template ≤ 7) | whatever resolves | `auth` verifies its scratch copy and proceeds, `pinned = "none"`, no pin inserted; `resource`/`migration` run no verification and write no evidence | 0 | `auth`: as the row above; `resource`/`migration`: `[verified_with]` untouched, now historical by FR-012-5d | `the project pins nothing` (on `auth`); once, that no toolchain action exists |
| A record with `record_version` newer than the reader | — | refused before any plan | **3** `record_unsupported`, `details.record_version`, `details.supported` (U-1) | — | the versions, and "rebuild the generator" |
...
Body changes, section by section: **Verbatim files** — the sentence "reads **nothing** from the framework checkout … except two files it validates … and one it copies" becomes "three files it validates (`Cargo.toml` and `rust-toolchain.toml`, both parsed and neither evaluated; `Cargo.lock`) and one it copies (`Cargo.lock`)"; **Starter sets** — a new group row `toolchain | at renvor new, or when the record declares a pin | rust-toolchain.toml, and the rust-version line of Cargo.toml`; **Generated-on-demand files** — `rustfmt` runs under the sealed environment of C-5 1.2.0; **Snapshot stability policy** — `[toolchain]` joins the pinned set, `[verified_with]` joins `Cargo.lock` outside it, the record's path stays pinned; **The provenance record** — the version-2 layout of §5.2, the reader rule of FR-012-5b/c, the freshness rule of FR-012-5d, and the write rule of FR-012-5a.

### 6.2 C-5 `generation-transaction.md` → 1.2.0 (provisional)

Status line addition: *1.2.0 (Phase 012, L-2): the sealed environment forces `RUSTUP_AUTO_INSTALL=0`, no longer passes `RUSTUP_DIST_SERVER` or `RUSTUP_UPDATE_ROOT`, refuses rustup below 1.28.1, an unidentifiable proxy, and a pinned-but-absent toolchain by name before step 2, records the compiler observed executing the checks (release, commit, host; override and wrapper presence only), and applies the same seal to `rustfmt` at generation and to `doctor`'s probes; `generate auth` stages its scratch copy beside the project so the project's toolchain selection is preserved. The protocol, atomicity, and residue rules are unchanged; the seal is not a sandbox for trusted wrappers or build scripts.* Body: "What 'verify before placing' means" gains the identification and resolution probes (FR-012-7a/7b — identify before invoking; nothing runs a proxy in a pinned directory until its rustup is known to honour the no-install guarantee), the evidence rule of FR-012-7d (the compiler observed executing the checks, taken from the checks themselves, with exclusively created storage), and the not-a-sandbox sentence; "Residue" gains the sibling scratch copy's name.

### 6.3 C-1 `command-surface.md` → 1.5.0 (provisional)

Additions, no exit code or stream rule changed: the `tool_missing` details of FR-012-7a/b (`details.tool` values `rustup >= 1.28.1`, `rustup toolchain <channel>`, `rustc >= <msrv>`, a component name; `details.remedy`); the `--framework-path` validation reasons of FR-012-1 (`toolchain_pin_malformed`, `toolchain_pin_unsupported`, `toolchain_pin_below_msrv`, `msrv_unreadable`); `project_verification_failed` reasons `compiler_identity_unreadable` and `toolchain_resolution_diverged`; the new refusal `record_unsupported` (**exit 3**, U-1 approved 2026-09-07 — an unsupported input record is a validation failure, not a missing tool; the same reason string and `details` keys as C-2's registry, the fixture, and the help text); the FR-012-8 stderr notice (one line, stderr, never stdout); `renvor check`'s two tables and the `historical` marker; `renvor doctor`'s toolchain section.

### 6.4 C-2 `json-output.md` — additive, no `schemaVersion` bump

`data.toolchain`, `data.verified_with` (with `historical`), and `data.doctor.toolchain` are additive fields; `record_unsupported` (exit 3) joins the error-code registry as an addition, with the same `details.record_version`/`details.supported` keys the fixture and the help text carry. C-2's own rule: adding is additive, removing is breaking. **The TOML record's `record_version` is a different axis** — the format version of a file the generator owns, governed by C-4 — and is not a `schemaVersion` change.
...
| **ADR-0040** (proposed) | Documentation versioning and the framework → documentation artifact handoff (§10.6), including the prerelease-snapshot rule of U-7 and the amendment text for `PLAN.md` §26.7/§26.8 that FR-012-D15 records | a lasting cross-repository commitment under `PLAN.md` §26.7/§26.8, which the decision amends |
...
The 2026-09-06 draft numbered the explicit behaviour table §7 and the compatibility assessment §8; the carry-over plan cites the brief's §3–§8 (its §2.5 and its D-L2-1 row). In this revision the behaviour table is §5.8 and the assessment keeps its number, §8, so those citations still land.
...
| **FR-012-D14** | **The documentation repository consumes it and nothing else.** A workflow `import-framework-artifact.yml` in `renvor-docs` (`workflow_dispatch` with the framework run id, the **full** SHA, and the expected digest) downloads the artifact and **verifies all four of FR-012-D15's conditions before unpacking anything** — the digest, the source repository, the source commit, and the producing workflow identity — then unpacks rustdoc under `static/api/<sha7>/`, regenerates `docs/reference/cli/*.mdx`, `docs/reference/json-schema.mdx`, `docs/limitations.mdx`, `docs/_stamp.mdx`, and the version label, commits the retention material of FR-012-D15, and opens a pull request. **No human copies framework content** (`PLAN.md` §26.8); the synchronisation is this recorded, versioned, automated step. |
| **FR-012-D15** | **Prerelease immutability (U-7, approved 2026-09-07 — a decision, no longer an assumption).** No release tag exists and none may be created in this phase. For prerelease documentation, a framework artifact addressed by its **full commit SHA and its content digest**, with **verified provenance**, satisfies the immutable-input requirement. **The importer verifies, and fails closed on, all four**: the archive's SHA-256 equals the digest given at dispatch and the one in `SHA256SUMS`; the attestation's source repository is `renvor-rs/renvor`; the attestation's source commit is the full expected SHA; the producing workflow identity is `renvor-rs/renvor/.github/workflows/docs-artifact.yml` (`gh attestation verify <archive> --repo renvor-rs/renvor --signer-workflow renvor-rs/renvor/.github/workflows/docs-artifact.yml --format json`, with the verified subject digest, source commit, and workflow fields compared to the expected values by the script, not by eye). **A filename, a short SHA, or a successful `--repo` verification alone is insufficient** and is a test failure (T-012-36's controls: a tampered archive, an attestation from another repository, from another commit, or from another workflow of the same repository, and a filename-only match are each refused). **Retention, for the documented period**: the small material — `manifest.json`, `SHA256SUMS`, the attestation bundle, and the verification output — is committed in `renvor-docs` under `evidence/framework-artifacts/<sha>/` for the life of the documentation version built from it; the archive itself is retained as an artifact of the producing run **and** of the importing run, each with `retention-days` set explicitly to the repository maximum (90 days on a public repository), and `manifest.json` records both expiry dates. **Never a silent substitute**: after expiry or deletion, a rebuilt artifact — even at the same SHA — has its own digest, its own `evidence/framework-artifacts/<sha>/` entry, and its own import pull request; a page stamp never changes without an import. **Stated on every page**: the stamp reads *prerelease snapshot `<sha7>` (digest `<first 12 hex of sha256>`) — not a signed release*, and the same workflows take a tag input the day one exists. **Policy amendment, recorded explicitly**: `PLAN.md` §26.7 names "a signed release tag or the published crate" and §26.8 "a release tag, a published crate, or a release asset addressed by digest"; T-012-33 proposes, in ADR-0040's batch, the sentence to add to both — *"Before the first release, a prerelease snapshot addressed by full commit SHA and content digest, with provenance verified against the producing workflow, is an accepted artifact; every page built from it states that it is a prerelease snapshot and not a signed release."* — accepted only by the maintainer (A-2). |
...
| Handoff | produces `renvor-docs-inputs-<sha>.tar.zst` + `SHA256SUMS` + attestation; retains the archive for the documented period | consumes by run id + full SHA + digest; verifies digest, source repository, source commit, and producing workflow identity (FR-012-D15); retains the verification material; regenerates; opens a PR | consumes the published image digest |
...
- **Scope**: PostgreSQL 17.11 with `ssl=on` and a `hostssl` rule; MySQL 8.4.11 with a certificate and `--require_secure_transport=ON`; the same ephemeral CA as §9; **certificate and hostname verification** on both engines through both adapters — SQLx `PgSslMode::VerifyFull` with the root certificate and `MySqlSslMode::VerifyIdentity` with the CA, and the same through SeaORM's `ConnectOptions`; the four rows positively; the negative cases (unknown issuer, hostname mismatch, a plaintext-only server under a verifying mode) per engine per adapter, each failing closed with `DatabaseError`'s category and never downgrading; the trust-source control of SR-012-11; the `verify-full` starter boot. **First task (U-9, approved 2026-09-07: measurement first; no `ConnectionString` redesign in the TLS proof)**: T-012-26 measures whether the URL parameters the drivers document (`sslmode=verify-full` with `sslrootcert=<path>` for PostgreSQL; `ssl-mode=VERIFY_IDENTITY` with `ssl-ca=<path>` for MySQL) carry certificate **and** hostname verification to both engines through both adapters when they pass through `renvor_database::ConnectionString` as it is today — the newtype hands the string to the driver verbatim through its one `expose()` call site, so the measurement is of what `renvor-sqlx` and `renvor-seaorm` do between `expose()` and the connection, and of the drivers' URL parsers. The outcome is a table per engine × adapter × parameter: *reaches the driver and is enforced* (the negative cases above fail closed under it) or *does not*. **If they work**, the bounded TLS proof uses them, and 010/L-15 (credentials inside the URL) stays open as it is. **If a required setting cannot reach a driver**, T-012-26 reports the exact gap — which parameter, which adapter, what the driver did instead — and proposes the smallest API change **separately**, as its own proposal beside 010/L-15's proposed-ADR target; B3 does not fold a `ConnectionString` redesign in, and verification is never downgraded to make a case pass (`verify-ca`/`VERIFY_CA` in place of full verification is a reported gap, not a proof). **No credential-bearing URL is printed** by any test, log, evidence file, or job summary: the type's fixed marker is the only rendering, and the tests plant a marker password and assert its absence in every stream.
...
| 010/L-15 | same | database credentials inside the URL | **retained** (U-9, approved 2026-09-07): the TLS proof uses the existing URL parameters if T-012-26 measures that they reach both drivers; the type is not redesigned in B3; a gap is reported and the smallest API change proposed separately, beside this row's proposed-ADR target |
...
- **Round 2 (2026-09-07)**: the capacity figures in the task plan §3 were measured on this machine at 2026-09-06 23:42 UTC with `du -sk` and `df -k` (`framework-phase-011/target` and its `starter-matrix`/`debug` children, `framework-wi-012-4/target/tc-*`, the other worktrees' targets, `~/.cargo/registry`, `~/.rustup/toolchains`, `docker system df`); nothing was deleted. `crates/renvor-database/src/pool.rs` (`ConnectionString`, `expose()`) and `.github/workflows/ci.yml` (the `verify` job's toolchain steps) were re-read at `7281e4f` for U-9 and U-10. `PLAN.md` §26.7 and §26.8 were re-read at `7281e4f` for the amendment text of FR-012-D15.
