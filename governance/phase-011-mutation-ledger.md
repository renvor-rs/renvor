...
**Total**: **64 scripted mutations, 64 killed by a named test**, plus one by history (M-H9, the
...
| I — the FR-048 decision: `--overwrite-unchanged` (2026-09-05, in the worktree: ten on the committed `2b3e4a8`, then all twelve on the committed `61a43af`) | 12 | 12 | 1 first form (M-I1 was a compile error, replaced by a compiling edit and killed) | `mutations-i.sh` (scratch); `mutations-i.log` and `mutations-i-<id>.out` (the `61a43af` run), `mutations-i-2b3e4a8.log` (the earlier run's log); every kill is a named test failing, none a compile error — checked per output |
...
### Batch I — the FR-048 decision, `--overwrite-unchanged` (2026-09-05, in the worktree: M-I1 … M-I8 on the committed `2b3e4a8`, all twelve on the committed `61a43af`)
...
| M-I9 | the refusal no longer lists what the plan would have created (`Action::Write` dropped from the listing) | `tests/generate.rs::a_changed_file_is_refused_with_the_flag_…` (`details.write`) |
| M-I10 | under the flag the refusal no longer lists what would be regenerated (`Action::Regenerate` dropped) | `apply::tests::absent_identical_untouched_and_modified_are_the_four_cases` (`details.regenerate`) |
