...
| # | Severity | Batch | Task | Status after validation | Regression evidence |
|---|---|---|---|---|---|
| L-01 | P1 | G | #48 | `needs_fixes` → fixed → **`validated`** | 15 tests. Validation found the success test asserted **exit 0 only** — a relay that printed nothing would have passed it. Now asserts the parsed payload |
| L-02 | P1 | F | #49 | **`validated`** | 4 tests over a real socket. Its bound assertion was additionally tightened from 5 s to `BUDGET * 5` |
| L-03 | P1 | A | #50 | **`validated`** | 5 tests; ordering read from recorded events |
| L-04 | P1 | A | #51 | **`validated`** | 3 tests |
| L-05 | P1 | C | #52 | **`validated`** | 5 tests |
| L-06 | P1 | B | #53 | `needs_fixes` → fixed → **`validated`** | Preflight asserted allow-**methods** but not allow-**headers**; the mirroring path was never exercised. Now asserted |
| L-07 | P1 | D | #54 | **`validated`** | 4 tests |
...
— and its comment claimed a segment-injecting router would fail it, which was false. A control that
cannot fail is worse than no control, because it is counted. It now drives the same reporting
handler the parameterised route uses, on a static path, and asserts the sentinel.
...
implied coverage their bodies did not deliver. That is the same defect class as an overclaiming
evidence row, one level down, and it is exactly what an independent pass is for.

**Two were proven by mutation rather than argued.** Deleting the line under test left the entire
suite green in both cases; after the fix, the new test is the only one that fails. A regression test
that cannot fail is not evidence, and the mutation is what tells the two apart.
...
### `validated` is the ceiling reached here

**Corrected 2026-08-23.** This heading previously read *"`coding_done` is the ceiling reached here"*
and stated *"No row above is `validated`"*. That was true when it was written and became false three
validation rounds later, while the disposition table above already said so. The document asserted
both states at once, which left the status of all twelve unreadable in practice. The post-remediation
requirements review found it.

All twelve are `validated`. None is `complete`. Under the five-status rule an implementation agent's
furthest reach is `coding_done`; the validation agent moved each row to `validated` independently;
`complete` is a human decision that has not been taken.
