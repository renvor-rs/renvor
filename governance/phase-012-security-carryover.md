...
**Revised**: 2026-09-06, to apply the maintainer's decision **D-L2-0** (§2.8); revised again the same day for **D-L2-1** (the mechanism, decided; everything it leaves open is specified and requested in `phase-012-specification-and-decision-brief.md`). Everything else below remains a proposal. **Revised 2026-09-07**: every remaining decision — **D-L1-1 … D-L1-7** and **D-L2-2 … D-L2-10** — was taken by the maintainer with corrections and is recorded in the brief's §4; the *Proposal* and *Decision needed* labels below are kept as the dated record of what was proposed, and the brief supersedes them where the two differ
...
The row is quoted as it stands on `main`; this plan does not edit it. Its target names **"C-4 1.2.0"**, which is historical: on 2026-09-06 the maintainer decided (D-L2-0, §2.8) that the number the row names is the revision C-4 **already** carries, and that L-2's corrected target is **C-4 1.3.0, provisionally** — the next revision — subject to the compatibility assessment D-L2-0 requires once the toolchain behaviour is specified.
...
| ID | **L-2** (Phase 011) |
| Owner | `renvor-cli` |
| Target | Phase 012, as a revision of contract C-4 (`contracts/template-contract.md`). As written in the row: **C-4 1.2.0** (historical, preserved above). Corrected under D-L2-0 (decided 2026-09-06): **C-4 1.3.0, provisional** — a minor revision is not forced if the compatibility assessment finds that the specified toolchain behaviour requires a major one |
...
- **which toolchain verified a given placed project** — nothing records it (`.renvor/generated.toml` has no such field, `renvor.toml` neither), so an author cannot tell, and a later `renvor generate` cannot check; *(note 2026-09-07, round 3: the brief's amended FR-012-7d records what was launched and queried, and records a cached verification as observed-identity-unavailable — the gap closes for launched runs and is stated, not hidden, for cached ones)*
...
The mechanism is decided (D-L2-1, 2026-09-06: a rendered `rust-toolchain.toml` **and** `rust-version` in `Cargo.toml`, with `verified_with` recording the compiler actually used). The criteria below were written for that shape; `phase-012-specification-and-decision-brief.md` §5–§8 carries them into a specification and supersedes them where the two differ.

| # | Criterion |
|---|---|
| **AC-L2-1** | **C-4 is revised** (provisionally as **1.3.0** per D-L2-0; the number is fixed only after the compatibility assessment D-L2-0 requires, and becomes a major revision if that assessment says so) to state what a generated tree declares about its toolchain, for both the skeleton and the starter, and what `renvor new` and `renvor generate` do when the declared toolchain is absent or incompatible. `contracts/generation-transaction.md` gains one sentence saying the seal's `PATH`/`RUSTUP_TOOLCHAIN` pass-through no longer decides the compiler alone. |
...
| **AC-L2-4** | The record says which toolchain verified the tree: `.renvor/generated.toml` gains a `verified_with` line (the `rustc --version` string), written after verification with the digests; `renvor check` reports it; the snapshot policy pins the key, not the value (as it does for `Cargo.lock`). **Note (2026-09-07, round 3)**: superseded in kind by the brief's FR-012-7d as amended by the maintainer — the record carries launch observation plus queried identity (release, commit, host as separate fields, not the `rustc --version` string), and a positively cached verification records the observed identity as unavailable rather than naming a compiler. |
...
### 2.8 Dependencies and decisions the maintainer must take — *Decision needed* (D-L2-0 and D-L2-1 **decided** 2026-09-06; D-L2-2 to D-L2-7 open, restated with recommendations in the brief) *(Dated note, 2026-09-07: D-L2-2 to D-L2-7 **decided** with corrections, and D-L2-8 to D-L2-10 added and decided the same day — brief §4.1; the proposals below are the dated record)*

| # | Decision | Proposal (not a decision), unless the cell is marked **DECIDED** |
|---|---|---|
| **D-L2-0** | **The contract version to target.** L-2 names "C-4 1.2.0". On `main` at `ab701d2`, `contracts/template-contract.md`'s status text already recorded a **1.2.0** revision (2026-09-05, the correction round) while its front-matter `version` still read **1.1.0**; `contracts/command-surface.md` had the same shape (`1.3.0` in front matter, a `1.4.0` revision in the status text). Whether the front matter or the status text was authoritative decided whether L-2's target was "1.2.0" or "the next revision, 1.3.0" | **DECIDED 2026-09-06 (maintainer), applied in this pull request.** (1) **Metadata correction, verified against Git history before editing.** `template-contract.md` `version` 1.1.0 → **1.2.0**: the 1.2.0 status text and the body rules it names ("Three rules were made explicit by the Phase 011 correction round") landed in `f6305a7` (2026-09-05) without the field, whose last change was `9dd5bc4` (1.0.0 → 1.1.0); `phase-011-evidence.md` already cites "`template-contract.md` 1.2.0". `command-surface.md` `version` 1.3.0 → **1.4.0**: the 1.4.0 status text landed in `2b3e4a8` (2026-09-05) together with the behaviour it describes (`--overwrite-unchanged` in `crates/renvor-cli/src/generate/apply.rs` and `crates/renvor-cli/src/commands/generate.rs`, pinned by `crates/renvor-cli/tests/generate.rs::a_regenerable_file_is_refused_without_the_flag_and_replaced_only_with_it`) without the field, whose last change was `f6305a7` (1.2.0 → 1.3.0); `phase-011-review-record.md` already cites "`command-surface.md` 1.4.0". Each field now equals the revision its own status text and history document. This corrects the field; it is **not a new contract revision and not a rule that narrative text always overrides front matter** — each case was checked against history on its own. (2) **L-2's target is C-4 1.3.0, provisionally** — the next revision after the corrected 1.2.0. The row's own "C-4 1.2.0" stays verbatim in §2.1 with this correction beside it. (3) **A compatibility assessment is required** when the actual toolchain behaviour is specified (D-L2-1 onward): a minor version is not forced if the change requires a major revision under the contract's own versioning. (4) **Nothing else is decided**: D-L2-1 to D-L2-7 and every D-L1 row stay proposals, L-1 and L-2 stay open, and this decision approves no implementation |
| **D-L2-1** | **The mechanism**: (a) a rendered `rust-toolchain.toml` (rustup selects the compiler; needs rustup); (b) `rust-version` in `Cargo.toml` (cargo refuses an *older* compiler only; a newer one still runs); (c) both; (d) record-only (`verified_with` in the record, no pin) | **DECIDED 2026-09-06 (maintainer): (c) both — a generated `rust-toolchain.toml` and `rust-version` in `Cargo.toml` — with `verified_with` recording the compiler actually used. The mechanism only.** Toolchain selection, override handling, operation without rustup, and compatibility remain to be specified together: `phase-012-specification-and-decision-brief.md` §3–§8 specifies them and its D-L2-2 to D-L2-10 request the remaining choices. This decision approves no implementation and closes nothing; L-2 stays open **Note (2026-09-07, round 3)**: "recording the compiler actually used" is superseded by the brief's FR-012-7d wording — launch observation plus queried identity, never proof of execution through a wrapper; a cached run is recorded as such. |
...
1. **A framework-backed starter with `--seed-data` and `--auth none` fails its own pre-placement `cargo fmt --check`** on `src/seed.rs` (rustfmt wants `SeedDeclaration::new(…)` broken over lines), so `renvor new … --database postgres --example-domain --seed-data --capabilities storage --framework-path <checkout> --yes` is refused with `project_verification_failed` ("a defect in renvor's templates") and nothing is placed. Reproduced on both ORMs and on both toolchains (`rustfmt` 1.8.0 under 1.94.0, 1.9.0 under 1.97.1); the same command without `--seed-data`, or with `--auth session --capabilities mail`, succeeds. No census row renders seeds without auth (`FULL` is the only flag set with `--seed-data`, and every `FULL` row has auth) — the "combination no row renders" shape of L-16. The skeleton's seeded variant is unaffected (it uses `templates/src_seed.rs.j2`, a different file). Reproduction and controls: the cheat sheet's verification record for 2026-09-06.
   - **Reproduction and evidence**: the command above on `main` at `ab701d2`; the JSON envelopes retained out of repository under `renvor-blog-api-cheatsheet-evidence/2026-09-06-ab701d2/logs/` — `defect-1-repro.json` (the refusal), `defect-1-control-noseed.json`, `defect-1-control-auth.json`, `defect-1-control-seaorm.json`, `defect-1-control-toolchain-1.94.0.json`, and the second round `D-ctl-seeded.json`, `D-ctl-unseeded.json`, `D-ctl-seeded-auth.json`, `D-ctl-seeded-sea.json`, `D-ctl-seeded-194.json` (driver log `D-controls.log`). The template is `crates/renvor-cli/templates/starter/src_seed.rs.j2`.
   - **Owner** — *Proposal*: `renvor-cli` (the starter templates and the census in `crates/renvor-cli/tests/starter_matrix.rs`).
   - **Acceptance criteria** — *Proposal*: (a) the starter's seed template renders `rustfmt`-stable output for `--auth none` and `--auth session` alike, on both ORMs; (b) the census gains a row that renders `--seed-data` **without** auth — the L-16 shape, a combination no row renders is unproven — and that row passes the pre-placement `cargo fmt --check` on both toolchain legs; (c) negative control: with the template change reverted, that row fails at the formatting check with the same `project_verification_failed` reason, proving the row exercises the defect; (d) the seeded-with-auth rows stay byte-identical in their snapshots, or move with the template version under C-4's snapshot policy.
2. **Two contract front-matter versions lagged their own status text** (`template-contract.md` 1.1.0 vs 1.2.0; `command-surface.md` 1.3.0 vs 1.4.0). **Corrected in this pull request under D-L2-0** (decided 2026-09-06; the history check is recorded in the D-L2-0 row and in §5). A metadata correction, not a contract revision. Nothing remains for Phase 012 on this item.
...
4. **A generated starter's `/metrics` renders only the jobs families.** Observed 2026-09-06 on a `cache,jobs,mail,storage,observability` starter after cache reads, a file write, and a mail submission: `process_start_time_seconds` and the six `renvor_jobs_*` families, nothing from `renvor_cache_*`, `renvor_mail_*`, or `renvor_storage_*`. The starter's `src/capabilities/jobs.rs` takes the metrics `Registry`; the cache, mail, and storage modules construct their providers without it (`crates/renvor-cli/templates/starter/src_capabilities_{cache,mail,storage}.rs.j2`), so the counters the capabilities contract names are not rendered by the starter's own route. An observation, not a security row; the capabilities crates' own suites still prove the counters.
   - **Reproduction and evidence**: `GET /metrics` on a `cache,jobs,mail,storage,observability` starter after one cache read, one storage write, and one mail submission; the retained scrape is `A2-g13m-metrics.txt` under `renvor-blog-api-cheatsheet-evidence/2026-09-06-ab701d2/logs/` (present: `process_start_time_seconds` and six `renvor_jobs_*` families; absent: every `renvor_cache_*`, `renvor_mail_*`, and `renvor_storage_*` family). Templates: `crates/renvor-cli/templates/starter/src_capabilities_{cache,mail,storage}.rs.j2` against `src_capabilities_jobs.rs.j2`, the one that takes the `Registry`.
   - **Owner** — *Proposal*: `renvor-cli` (the starter templates); the capability crates are not implicated.
   - **Acceptance criteria** — *Proposal*: (a) the starter's cache, mail, and storage providers are constructed with the same metrics `Registry` the jobs module receives, so `/metrics` renders the families the capabilities contract names for every selected capability after one operation each; (b) the generated starter test, or a census row, asserts each family's presence after the corresponding operation on a full-capabilities starter and — negative control — the absence of a capability's families on a starter that did not select it; (c) the observability section of the generated README states which families the starter renders.
5. **`a_hostile_argv0_does_not_reach_the_terminal_raw` (`crates/renvor-cli/tests/hostile.rs`) failed once in CI on a documentation-only change**: the `verify (1.94.0)` job of this pull request's run on `2f74962` (2026-09-06, `ubuntu-latest`) panicked at the `expect("the planted binary runs")` with `Os { code: 26, kind: ExecutableFileBusy, message: "Text file busy" }`, step `[4/9] tests`; the same test passed three consecutive local runs on 1.94.0 (macOS) and every earlier CI run of the tree. The test copies the `renvor` executable into a temporary directory and executes the copy at once, in a test binary whose other tests spawn `renvor` children concurrently — the shape in which Linux answers `ETXTBSY` when a fork inherits the still-open write descriptor of the copy until its own exec. That is a hypothesis from the error and the test's shape, not a proven cause. It is recorded here because the test is a terminal-injection control (a raw `ESC` from `argv[0]` must never reach a human stream): a control that flakes invites routine re-runs, and routine re-runs hide a real failure. A Phase 012 fix belongs in the test harness, not in the generator.
   - **Reproduction and evidence**: run `34017828678`, job `verify (1.94.0)` (<https://github.com/renvor-rs/renvor/actions/runs/34017828678/job/101444736622>), step `[4/9] tests`; the log excerpt is retained as `ci-pr63-2f74962-verify-1.94.0-hostile-etxtbsy.log` under `renvor-blog-api-cheatsheet-evidence/2026-09-06-ab701d2/logs/`. Not reproduced locally (three consecutive passes on 1.94.0, macOS) and not reproduced on the next run (`5acb492`, every leg green). The cause stated above is a **hypothesis**, not a finding.
   - **Owner** — *Proposal*: the `renvor-cli` test harness (`crates/renvor-cli/tests/hostile.rs`), not the generator.
   - **Acceptance criteria** — *Proposal*: (a) the cause is established on Linux before any fix — the hypothesis confirmed (for example by a looped run of the test binary beside a concurrent spawner, or by tracing which descriptor is still open at the exec) or refuted and replaced; (b) the harness change matches the confirmed cause (for the hypothesis: flush and close the copy before it is executed and retry a bounded number of times on `ETXTBSY`, or run the planted-binary tests serially); (c) a soak of the test on `ubuntu-latest` on both toolchain legs passes a stated number of consecutive runs without a re-run; (d) negative control: the assertion itself is untouched — a planted `argv[0]` carrying a raw `ESC` still fails the test — so the control keeps its teeth.
...
- The D-L2-0 correction (2026-09-06) was verified against Git history before either field was
  edited: `git log -S` located the commit that introduced each status-text revision (`f6305a7`
  for `template-contract.md` 1.2.0, `2b3e4a8` for `command-surface.md` 1.4.0); `git log -L` on
  the two front-matter lines showed that neither commit touched the `version` of the file whose
  revision it introduced (those fields were last changed in `9dd5bc4` and `f6305a7`
  respectively); and the body text and code each revision names were
  located on `main` at `ab701d2`. No workflow or `xtask` step names the two contracts or their
  front matter, so no gate depended on the lagging value.
