...
Each batch is one or more pull requests; each pull request runs the full framework gate (`verify` ×2, `platform` ×4, `security`, `docs`, CodeQL) and, from B2 on, the `tls` job. A batch that changes source runs `cargo xtask verify` locally before its pull request opens (`CONTRIBUTING.md`), on both toolchains as the phase evidence records have done (their legs A and B); local stable is 1.97.1 on this machine, so only CI's stable proves a 1.98 lint. Template-version bumps are serialised: one bump per merged batch.
