...
| **Required checks for `renvor-rs/renvor-infra`** — its ruleset requires pull requests and signed commits but verifies nothing, because it has no CI (limitation R-9). **DISCHARGED LATE 2026-08-17T20:42:25Z, deadline MISSED.** The `validate` check (GitHub Actions app 15368, strict) is now required. It was not required when the first manifest merged at 2026-08-17T16:31:44Z, nor for the six pull requests after it | Ahmed Anbar | **Before the first manifest is merged** — *not met* | Any pull request adding a manifest | Discharged when CI exists and its checks are required — **met 2026-08-17T20:42:25Z, 4 h 10 min 41 s after the deadline** |
