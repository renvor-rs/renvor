...
| `destination_exists` | 3 | Something is already at the destination. `details.found` is `directory`, `file`, `symlink`, or `other`. |
...
- **The destination must not exist, and `renvor` never deletes one.** An empty directory, a
  non-empty directory, a file, and a symbolic link are all refused with `destination_exists`, before
  anything is staged. If renvor cannot even tell what is there, it refuses rather than guessing.
  Nothing you already have is deleted, renamed, `chmod`-ed, replaced, or restored.
