...
- **An existing *empty* destination directory is removed and replaced.** FR-013 refuses a
  destination that "exists and is **not** empty", so an existing empty one is a legal target — and
  the way `renvor` uses it is to `remove_dir` it and rename staging into its place. The directory
  you get back is a **new** one: its permissions, ownership, and extended attributes are the
  defaults, not the ones you set. A deliberate `chmod 700` on an empty destination will come back
  as `755`. If the rename fails, the removed directory is recreated.
