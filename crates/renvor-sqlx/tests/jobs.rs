...
macro_rules! jobs_suite {
    ($module:ident, $feature:literal, $driver:ty, $connect:path, $run:ident, $url:expr, $engine:literal) => {
        ...
        mod $module {
            ...
            use renvor_sqlx::jobs::$module::SqlxJobStore;
            ...
            struct Fixture {
                ...
                store: Arc<SqlxJobStore>,
            }

            impl JobsFixture for Fixture {
                type Store = SqlxJobStore;
            ...
            async fn the_shared_jobs_contract_holds() {
                ...
                let store = Arc::new(SqlxJobStore::new(
                    database.pool().clone(),
                    bounds,
            ...
            async fn the_job_schema_applies_and_a_second_run_changes_nothing() {
                ...
                let count: i64 = sqlx::query_scalar(sqlx::AssertSqlSafe(
                    "SELECT COUNT(*) FROM rv_job".to_owned(),
                ))
