...
pub fn run(reporter: &Reporter, path: &std::path::Path) -> Result<Exit, CliError> {
    ...
    // Validated, not merely recorded — for the same reason `target` is. A manifest naming a
    // transport this version does not ship describes a project it could not have generated.
    if manifest.project.transport != "rest" {
        return Err(invalid(
            "project.transport",
    ...
    let human = crate::output::layout::Report::new()
        ...
        .row("Transport", manifest.project.transport.clone())
