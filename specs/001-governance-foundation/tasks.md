...
> **WARNING TO ANY TOOL OR READER COUNTING CHECKBOXES.** T114 below is written `- [X]` because
> Markdown checkboxes have only two states and this task needs a third. **`[X]` here means
> CANCELLED, not completed.** A mechanical `grep -cE '^- \[[xX]\] '` over this file therefore
> returns **102**, which is **101 completed plus 1 cancelled** — it is **not** 102 completed
> tasks, and T114's backup-and-restore requirements were never met. The authoritative reading
> is **101 completed, 1 cancelled, 12 open, 114 total**. Whether to move T114 out of checkbox
> grammar entirely, so the mechanical count stops disagreeing with the prose, is an open
> governance question for the maintainer.
