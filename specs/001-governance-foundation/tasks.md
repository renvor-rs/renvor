...
- [ ] T054 [US4] **FIRST PUSH GATE**: push initial content to the public repository — permitted only after T022 (names confirmed), T044 (both licence texts), T045 (security contact live), and **T053** (current zero-finding scan) are all complete (FR-052)
- [ ] T055 [US4] Configure branch protection on `main`: pull request required, required status checks, **0** approvals under waiver W-001, and **no bypass permission on any account including administrators**. The four required check names are registered at T062 once the workflows producing them exist; until then protection intentionally references checks that are not yet reporting (analyze finding O2)
- [ ] T056 [P] [US4] Enable secret scanning with push protection, code scanning (CodeQL, Rust GA since 2025-10-14), dependency graph and alerts, and dependency review — all free on the public tier, so no cost-based waiver is acceptable
- [ ] T057 [P] [US4] Create `.github/workflows/ci.yml` running `cargo xtask verify` across the `1.94.0` and `stable` matrix, with top-level `permissions: contents: read`
- [ ] T058 [P] [US4] Create `.github/workflows/security.yml` running cargo-deny, dependency review, CodeQL, and clippy→SARIF upload, with `security-events: write` scoped to the upload job only
- [ ] T059 [P] [US4] Create `.github/workflows/docs.yml` for the documentation build and link check, with `pages: write` and `id-token: write` scoped to the deploy job only
- [ ] T060 [P] [US4] Create `.github/dependabot.yml` covering the `cargo`, `github-actions`, and `npm` ecosystems
- [ ] T061 [P] [US4] Create `.github/ISSUE_TEMPLATE/` bug and feature templates plus `config.yml` routing security reports to the private path, and `.github/PULL_REQUEST_TEMPLATE.md` and `.github/RELEASE_TEMPLATE.md`
- [ ] T062 [US4] Pin every third-party action in `.github/workflows/` to a full 40-character commit SHA with a trailing `# vX.Y.Z` comment, then register the four required check names from contracts/verification-sequence.md — `verify (1.94.0)`, `verify (stable)`, `security`, `docs` — in branch protection (depends on T055, T057–T059)
- [ ] T063 [US4] Record the observed protection baseline in `governance/phase-001-evidence.md` against every field in data-model.md §Repository Protection Baseline, and confirm the waiver count is exactly one (SC-008)
...
- [ ] T070 [US6] Write `RELEASING.md` covering topological publish order with the index-availability wait, version immutability and yank-and-replace as the sole remedy, the least-scope bootstrap credential procedure with immediate revocation, and the evidence retention period
...
- [ ] T073 [US6] Create `.github/workflows/release-dry-run.yml` with `permissions: contents: read` and **no publish capability**, so it cannot publish even if invoked
- [ ] T074 [US6] Run `cargo package -p renvor --list` from a clean checkout and record the exact file list in `governance/phase-001-evidence.md`
- [ ] T075 [US6] Inspect that file list for secrets, local configuration, build output, and unintended assets; record the review outcome (FR-039)
- [ ] T076 [US6] Run `cargo package -p renvor` and `cargo publish --dry-run -p renvor`; record artifact path and `sha256` in `governance/phase-001-evidence.md`
- [ ] T077 [US6] Validate package metadata against contracts/package-metadata.md and confirm no publishable package carries a path-only dependency
- [ ] T078 [US6] Query the live registry and record **zero versions** for every intended name — positive evidence of non-publication, not an assertion that nothing was run (SC-010)
- [ ] T079 [US6] Confirm no long-lived registry credential exists in the repository, its workflows, or its secrets; record the check (FR-033)
