...
- [ ] T098 Decide and record the **website-code licence** and the **brand-asset usage terms** for V7; brand assets are NOT covered by the framework `MIT OR Apache-2.0` grant. `renvor-rs/renvor-site` **already exists** (created manually 2026-08-12), so this no longer blocks repository creation — it **blocks the first content commit or push to `renvor-rs/renvor-site`, and any public use of the site or its brand assets**. **Override recorded 2026-08-12** (evidence §3ag/§3ah): the maintainer authorised publishing the mark in the public framework README behind an explicit exclusion notice, and authorised the first `renvor-site` content commit. **T098 is NOT closed by that** — the licence and usage terms remain undecided, and the gates now block deployment rather than the first commit
- [ ] T099 Decide and record the **container registry** — GitHub Container Registry versus the GitLab registry already on the VPS — including the credential model. The private repositories **already exist and are empty**, so this no longer blocks their creation — it **blocks registry configuration, image publication, deployment workflows, and production deployment** (ADR-0006 unresolved question 1)
