...
- **FR-010**: A dependency and license policy MUST state which licenses are permitted, which require review, which are prohibited, and how security advisories and unmaintained dependencies are handled, including the outcome options available to a reviewer.
...
- **FR-046**: Verification and rehearsal runs MUST retain their build evidence for a stated period, so a reviewer can later reconstruct what was verified, on which platform, by whom, and when.
