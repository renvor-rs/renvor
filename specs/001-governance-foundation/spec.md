...
- **FR-005**: An accepted decision record MUST explain the intentional distinction between the product name and the installed executable name, and all documentation, tests, and examples MUST use the executable name consistently. *(Amended 2026-08-17 by **ADR-0010**, which removed the distinction: the product, the facade crate, and the executable now share the spelling `renvor`. The **first clause is satisfied by ADR-0010 itself**, which explains why the distinction ADR-0001 created was removed — the requirement is that the naming decision be justified in a record, and it is. The **second clause is unchanged and still binding**: documentation, tests, and examples must use the executable name consistently, and now do. This wording is left standing rather than rewritten because it is the accepted text of a closed phase; the amendment is recorded here and in ADR-0010's impact analysis.)*
