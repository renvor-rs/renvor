...
| **III — Package-first boundaries** | Maintained packages evaluated before custom infrastructure; custom choices need an accepted ADR | **Pass, and the gate is discharged by removal rather than by waiver.** [`research.md`](research.md) D6 **revision 2** adopts `cap-std` 4.0.2 after measurement falsified revision 1's two objections. No custom path-containment infrastructure remains, so no ADR is required and **none was created** |
...
| ~~Hand-composed path containment~~ | **Withdrawn.** See D6 revision 2 | `cap-std` 4.0.2 is **adopted**. Revision 1 deferred this to an ADR; measurement showed both of its objections were false for this design, and adopting the package removes the decision rather than deferring it |
