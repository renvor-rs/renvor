...
- The four transferred Phase 001 deployment gates (T102, T108, T109, T111) remain non-completed throughout and are out of scope here. **Corrected 2026-08-17 (T159, finding R4-5): two of the four are no longer accurate as written.** Phase 002's **T151** acted directly on the subject matter of **`001-T108`** ("DOCUMENTATION DEPLOYMENT GATE — resolve `image-size`") and **`001-T109`** (the `uuid` advisory `GHSA-w5hq-g745-h8pq`), removing all three advisories from the documentation site's resolved dependency graph. That was done under **explicit maintainer direction**, because PLAN.md §17.3 forbids accepting a phase with an open Critical or High finding and no waiver for one is available — a direction that overrides this assumption rather than being accommodated by it. **`001-T102` and `001-T111` remain untouched and out of scope.** Closing the two Phase 001 rows is Phase 001's bookkeeping and is **not** done here; it is recorded as an open item with an owner in `governance/phase-002-evidence.md`, because editing a closed phase's ledger from inside another phase's pull request is how two evidence packs come to disagree.
