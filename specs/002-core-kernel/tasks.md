...
- [x] T132 Push the exact final commit to `refs/heads/feat/phase-002-core-kernel` with a non-force refspec and open one non-draft pull request into `main`, stating scope, validation, waiver status, and named limitations. **Stop before merging** — *(reopened 2026-08-16 because the head and body were superseded by Phase 11, then again by Phase 12)* — **closed 2026-08-17 at T161**: the final head is pushed with an explicit non-force refspec, PR #19 is open, non-draft, based on `main`, and its body states the final verified scope, validation results, waiver status including **W-006**, and the named limitations. Merging is **not** part of this task and remains **T158**
...
- [x] T156 Run the full validation matrix on a clean committed tree: formatting and `git diff --check`, clippy with `-D warnings`, all workspace tests serially, doc tests, `cargo xtask verify` 11/11 on **1.94.0** and current stable, the all-target no-default-feature checks, `cargo deny`'s four checks, quickstart gates **0–15 individually**, Gate 12 under **bash 3.2 and zsh**, `actionlint`, the documentation frozen install and build, the release dry run with zero publication, secret scanning, and CodeQL. Verify the exact **FR-001…FR-044** and **SC-001…SC-022** mapping and that task IDs are contiguous and unique, and that **every task except the explicitly post-merge integration task is checked** — **closed**: `cargo xtask verify` **11/11** on both toolchains from a clean committed tree, clippy `-D warnings` clean, **344** tests across 28 targets serially with 0 failing, `cargo deny` 4/4, `actionlint` **0**, gates **16/16** with **170** tests executed across gates 2–11, Gate 12 passing in **bash 3.2 and zsh**, FR-001…FR-044 and SC-001…SC-022 all mapped, task IDs T001…T161 contiguous and unique.
  > **Corrected 2026-08-17 (T161).** This task previously claimed *"task IDs … all checked"*, which was false while T132, T157, and T158 were open, and self-referential besides — a validation task cannot truthfully assert the completeness of a list that contains itself and a task that has not yet happened. **T156 claims completion of implementation and pre-merge validation only.** It explicitly **excludes T158**, the event-dependent integration task, which cannot be true until PR #19 is merged and therefore cannot be asserted by anything inside PR #19. **One Phase 002 task — T158 — is deliberately open at merge time**, and that is the correct state, not an oversight. *(Updated 2026-08-17: T158 closed after the merge, in the separate evidence-only pull request. **T001…T161 are now all checked — 161 of 161** — and that statement is true for the first time here, on the far side of the event it describes, rather than asserted inside the pull request that had not yet caused it.)*
- [x] T157 Push the corrected exact head with a non-force refspec, update PR #19's body to the verified final facts and counts, and wait for every check on that exact head. Verify zero unresolved conversations, zero open CodeQL alerts, **the branch's dependency evidence** (below), and zero tags, releases, crates, deployments, or Phase 003 work — **closed 2026-08-17 at T161**
  > **Corrected 2026-08-17 (T161): this task previously required "zero open Critical/High dependency alerts" before merge, which is not satisfiable by anything this pull request can do.** GitHub computes Dependabot alerts from the **default branch only**. The fix for both High `image-size` advisories lives on this branch, so the repository-level counter cannot fall to zero until the merge itself lands the fixed tree on `main`. Requiring the post-condition of a merge as the pre-condition of that same merge is circular, and it was written that way by mistake rather than by policy. The requirement is therefore **split by the branch it is measurable on**, and neither half is weakened:
  >
  > **Pre-merge, on this branch — all four required, all met:**
  > 1. **zero vulnerable `image-size` or `uuid` packages in the pull request's resolved graph** — verified against the committed `docs/package-lock.json`, not against a claim;
  > 2. **`npm audit` reports 0 findings** on a frozen install from that lockfile;
  > 3. **`dependency-review` passes** on the exact head;
  > 4. **no finding dismissed, waived, or suppressed** — both High advisories were closed by *removal* because no fixed version exists at any release, recorded in **ADR-0009**; the Medium `uuid` finding was fixed by upgrade in the same change.
  >
  > **Expected and disclosed, not a failure:** GitHub's default-branch alert counter is expected to **remain at 2 High + 1 Medium until this pull request merges**. That number describes `main`, which still carries the vulnerable tree. It is not evidence about this branch and must not be read as one.
  >
  > **Post-merge — moved to T158, where it is measurable:** zero live open Critical/High Dependabot alerts after GitHub recomputes against the new `main`. **This is an acceptance condition, not a formality**: if the alerts do not close after recomputation, Phase 002 is not accepted and the actual blocker is reported instead.
...
- [x] T158 Re-verify immediately before merging — open, non-draft, MERGEABLE/CLEAN, base at the current live `main`, head the exact reviewed SHA, all checks settled successfully except documented event-inapplicable skips — then **squash-merge with `--match-head-commit`**, and verify afterwards that PR #19 is merged, that `main` points at the resulting integration commit, that the integration tree equals the reviewed source tree, that the signature is verified, that post-merge checks settle successfully, that **live Dependabot recomputes against the new `main` and reports zero open Critical/High alerts**, that the removed vulnerable packages are absent from the live-`main` lockfile, and that no crate, tag, release, deployment, or Phase 003 work was created
  > **This task is deliberately open, and must stay open inside PR #19 (recorded 2026-08-17 at T161).** Every assertion above is about an event that has not happened — the merge — and about state that only exists afterwards. **A task cannot record its own merge inside the pull request being merged**, and marking it complete prospectively would assert a verified merge commit, a verified integration tree, and a recomputed alert count that do not yet exist. That is precisely the class of claim this phase has spent four review rounds removing.
  >
  > **T158 is therefore completed in a separate, evidence-only closure pull request** opened from the new `main` after PR #19 merges, on branch `docs/close-phase-002`. That pull request records the merge SHA, `mergedAt`, the merge actor, tree equivalence, signature verification, post-merge check results, and the recomputed Dependabot state — all measured, none predicted — and moves Phase 002 from *conditional on integration* to *integrated and accepted*.
  >
  > **Until then Phase 002 is not accepted.** One open integration task is the honest representation of that, and it is visible rather than reconciled away.
  >
  > **Closed 2026-08-17, in the separate evidence-only pull request this row promised.** All 26 pre-merge conditions were reverified live immediately before merging, and the guarded squash merge ran with `--match-head-commit fae251c…` — no admin override, no auto-merge, no queue, no branch deletion, no custom merge text. Measured after the merge: PR #19 **MERGED** at **2026-08-17T07:23:53Z** by **AhmedAnbar** as **`8abc0f4168300f04b86e01b0dc0a4bfff2a15af9`**; `main` advanced from `19605e90…`; the integration commit has **one** parent and a tree **byte-identical** to the reviewed source tree `a5734b96…`; GitHub reports the signature **`verified: true`**; post-merge checks on `main` are **10 success, 1 event-inapplicable skip**; Dependabot recomputed **five seconds** after the merge to **0 open alerts, 0 Critical/High, 0 dismissed** — both High `image-size` advisories and the Medium `uuid` advisory all `fixed`; and the live-`main` lockfile carries **0** vulnerable `image-size` tarball entries. **0** crates, tags, releases, deployments, or Phase 003 artifacts were created. Full detail in `governance/phase-002-evidence.md` §Integration

---

## Phase 13: Governance Closure (T160–T161)

**Purpose**: record the maintainer's **W-006** decision of 2026-08-17, discharge its compensating
controls against **ADR-0009**, and repair the self-referential task ledger that Phase 12 left
behind. Both tasks are new and contiguous. **Neither hides its work inside an existing task**, and
neither rewrites historical evidence — corrections are dated and additive.

- [x] T160 Record **W-006** in every authoritative waiver location — `governance/waivers.md` (row, narrow-scope section, category table, axis table, recurring-obligation and expiry records), `GOVERNANCE.md`, the documentation-site governance mirror at `docs/docs/governance.mdx`, and `governance/phase-002-evidence.md` — with the approval by Ahmed Anbar on **2026-08-17**, the exact scope (**ADR-0009 only**), expiry **2027-02-11**, the compensating controls, the prohibition on describing any review as independent, and the requirement for genuine independent re-review. Discharge W-006's controls: run **two fresh clean-context advisory reviews of ADR-0009** — one requirements/package-governance, one security/supply-chain, both labelled **NON-INDEPENDENT** and **ADVISORY**, each returning enumerated findings or an explicit named "no findings" result; **disposition every finding individually with no grouping**; **fix every Critical, High, and Medium**; and only then move ADR-0009 from `proposed` to `accepted` with reviewer exactly `Ahmed Anbar — self-review under W-006` and a recorded review date, stating plainly that this is not independent review. **Record explicitly that W-006 is the third explicit reviewed exception in Phase 002, exceeding the ledger's expected maximum of two**, without extending W-004 or W-005 and without concealing that the underlying problem is the continued absence of a second qualified human reviewer — **closed**: W-006 recorded in `governance/waivers.md` (row, narrow-scope section, third-exception departure, category and axis tables, ruling, expiry ratchet), `GOVERNANCE.md`, `docs/docs/governance.mdx`, and `governance/phase-002-evidence.md`; **attempt 1 of the reviews recorded NOT PERFORMED** — both agents returned nothing, the third occurrence of that failure mode — and the W-004 remedy re-applied; attempt 2 returned **24 findings (2 CRITICAL, 10 MAJOR, 12 MINOR)**, every one dispositioned individually and **all 24 fixed, 0 refused**; ADR-0009 moved to `accepted` with reviewer `Ahmed Anbar — self-review under W-006` **only after** that, and quickstart **Gate 14e** added so both accepted Phase 002 records are enforced rather than one
- [x] T161 Repair the self-referential task ledger. **T132**: close it on the final pushed head and final PR body, removing the superseded-head wording. **T157**: split its dependency requirement by the branch each half is measurable on — branch-resolved-graph, `npm audit`, `dependency-review`, and no-dismissal pre-merge; live default-branch alert recomputation post-merge — and state that GitHub's counter is **expected** to remain at 2 High + 1 Medium until the fixed tree reaches `main`. **T156**: narrow its claim to implementation and pre-merge validation, explicitly excluding the event-dependent integration task, so it no longer asserts that every task is checked while one is open. **T158**: leave it **unchecked**, state that it is completed in a separate evidence-only closure pull request from the new `main`, and do not mark it prospectively complete. Reconcile task counts and evidence so that **exactly one** Phase 002 task — T158 — is visibly open at merge time — **closed**: T132 and T157 closed on the final head, T156 narrowed to implementation and pre-merge validation with T158 explicitly excluded, T158 left unchecked with its closure routed to a separate evidence-only pull request from the new `main`; task IDs **T001…T161** contiguous and unique, **160 checked, 1 open** at merge time — **161 of 161 checked** once T158 closed post-merge
