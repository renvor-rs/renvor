...
| 4 | Tests | `cargo test --workspace --all-features` with `RENVOR_TEST_STARTER_ROWS=none`, then the end-to-end route relay, then the row census — every one of the 82 required (row, test) pairs must report in, the fifteen starter rows included, which the census runs with `RENVOR_TEST_TARGET_DIR=target/starter-matrix` and for which it refuses `RENVOR_TEST_STARTER_ROWS` in its environment | Rust, both databases, Valkey, Mailpit |
