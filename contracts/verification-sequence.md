...
|---|---|---|---|
| 1 | Prerequisite probe | — | Rust (pinned), **a PostgreSQL and MySQL the census can reach, and a Valkey and Mailpit the capability suites can reach** (`RENVOR_TEST_VALKEY_URL` and `RENVOR_TEST_VALKEY_PASSWORD`, `RENVOR_TEST_SMTP_URL` with `RENVOR_TEST_SMTP_USERNAME` and `RENVOR_TEST_SMTP_PASSWORD`, `RENVOR_TEST_SMTP_API_URL`, `RENVOR_TEST_REQUIRE_CAPABILITIES=1`; a URL never carries the credential)  **The step names the compiler it probed** (2.4.0, AC-012-4): the `release` and `commit-hash` of `rustc -vV`, asked directly. A green sequence whose log does not say what compiled it cannot be audited — between 2026-08-11 and `7281e4f` the contexts named `stable` compiled the pinned 1.94.0 and every step still reported `ok`. |
| 2 | Formatting | `cargo fmt --all --check` | Rust |
| 3 | Lint | `cargo clippy --all-targets --all-features -- -D warnings` | Rust |
| 4 | Tests | `cargo test --workspace --all-features` with `RENVOR_TEST_STARTER_ROWS=none`, then the end-to-end route relay, then the row census — every one of the 86 required (row, test) pairs must report in, the nineteen starter rows included, which the census runs with `RENVOR_TEST_TARGET_DIR=target/starter-matrix` and for which it refuses `RENVOR_TEST_STARTER_ROWS` in its environment | Rust, both databases, Valkey, Mailpit |
